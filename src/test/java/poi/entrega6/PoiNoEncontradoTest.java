...
public class PoiNoEncontradoTest implements WithGlobalEntityManager {
	...
	/*
	 * Posicion posicion = new Posicion(40.417, 3.703); Direccion direccion =
	 * new Direccion(); LocalComercial kiosco = new
	 * LocalComercial("kiosco1",posicion,direccion,RadioCercania.Kiosco);
	 * 
	 * @Before public void setUp(){
	 * 
	 * direccion.setBarrio("Almagro"); direccion.setLocalidad("Chivilcoy");
	 * direccion.setCalle("Guemes"); direccion.setCodigoPostal("2222");
	 * 
	...
	public void poiNoEncontrado() {
		...
		LocalComercial kiosco = new LocalComercial("kiosco1", posicion,
				direccion, RadioCercania.Kiosco);
		...
		idRecuperado = entityManager()
				.createQuery(
						"from LocalComercial l where l.nombre like :nombre",
		...
		Assert.assertTrue(entityManager()
				.createQuery("from LocalComercial l where l.PoiId = :id",
						LocalComercial.class).setParameter("id", idRecuperado)
